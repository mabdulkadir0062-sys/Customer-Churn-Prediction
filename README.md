...
Predicting which customers are likely to churn, so a retention team can
prioritize outreach. Built with XGBoost and hyperparameter-tuned with
Optuna.
...
- **Learning rate hit the search ceiling** (0.08–0.10) — worth widening the
  search range on a rerun.
